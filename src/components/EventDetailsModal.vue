    ...
    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      ...
      <div class="bg-white w-[90%] max-w-3xl p-6 rounded-lg shadow-lg relative transform transition-all">
        ...
        <div class="flex items-center justify-between pb-3">
          <h2 v-if="props.selectedEvent?.summary" class="text-xl font-bold text-gray-800">
        ...
        <div class="mt-4 space-y-3 text-gray-700">
          ...
          <div class="rounded-md shadow-md p-5 text-md">
            <p >
            ...
            <p>
              <i class="bi bi-repeat mr-2 text-purple-500"></i> {{
...
const getFrequency = (freq, byDay, interval, wkst, until, count) => {
  ...
  let text = 'This event occurs ';

  switch (freq) {
    case 'DAILY':
      text += interval ? `every ${interval} days` : 'daily';
      break;
    ...
    case 'YEARLY':
      text += interval ? `every ${interval} years` : 'annually';
      break;
    ...
    default:
      if (byDay) {
        ...
        text += wkst > 1 ? ` every ${wkst} weeks` : ' every week';
        break
      }
