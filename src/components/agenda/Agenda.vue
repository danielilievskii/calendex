<template>
    ...
    <!-- Desktop Table View -->
    <div class="hidden md:block rounded-md border border-gray-200 shadow-lg ">
        ...
        <TableHeader>
          <TableRow class="bg-white hover:bg-gray-200/50">
            <TableHead
                ...
                :key="header.key"
                class="font-bold text-lg cursor-pointer px-4 py-4 text-left text-[#31776c]"
                :class="{
    ...
    <div class="md:hidden space-y-4">
      <div
          v-for="event in sortedEvents"
          :key="event.uid"
          class="bg-white rounded-lg border border-gray-200 shadow-md p-4"
      >
        <div class="flex items-start justify-between mb-3">
          <div class="flex items-center gap-2">
            <div
                ...
                :class="[colorStore.getBackgroundColor(event.calendarColor)]"
            ></div>
            <span class="text-sm text-gray-600 font-medium">{{ event.calendarName }}</span>
          </div>
          <Badge class="bg-[#52B788] flex-shrink-0">{{ event.duration || 'Unknown' }}</Badge>
        </div>

        <h3 class="font-bold text-lg text-black mb-2">{{ event.summary }}</h3>

        <div class="space-y-1 text-sm">
          <div class="flex items-start gap-2">
            <span class="text-gray-500 font-medium min-w-[70px]">Location:</span>
            <span class="text-gray-700">{{ event.location || "No location" }}</span>
          </div>
          <div class="flex items-start gap-2">
            <span class="text-gray-500 font-medium min-w-[70px]">Time:</span>
            <span class="text-gray-700">{{ formatFromStringToDateTime(event.startDateTime) }}</span>
          </div>
        </div>
      </div>

      <div v-if="calendarStore.filteredEvents.length === 0" class="text-center py-8 bg-white rounded-lg border border-gray-200">
